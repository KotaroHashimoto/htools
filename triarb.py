...
#        base += 'T' if base == 'USD' else ''
    ...
    def sell(self, symbol, amount, rate):
        pair = self.symbol2pair(symbol)
        fl = 3 if pair[:3] == 'USD' else 8
        return self.private.sell_limit(pair, round(amount, fl), round(rate, fl))

    def buy(self, symbol, amount, rate):
        pair = self.symbol2pair(symbol)
        fl = 3 if pair[:3] == 'USD' else 8
        return self.private.buy_limit(pair, round(amount, fl), round(rate, fl))
...
class Control():
    ...
    def report(self, init, amount):
        ...
        if init:
            r = range(int(24 * 3600 / (2 * INTERVAL * MAIL_INT)))
        ...
        pIndex = (Control.index + 1) % len(Control.BTC)
        ...
        r = 100.0 * (net / pNet - 1.0)
        vr = 100.0 * (vNet / pNet - 1.0)
        rNet = str(round(net, 1)) + ' USD (r:' + ('+' if 0 <= r else '') + str(round(r, 3)) + '%, v:' + ('+' if 0 <= vr else '') + str(round(vr, 3)) + '%)'
